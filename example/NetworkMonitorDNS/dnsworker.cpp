...
PCAP::IpAddress DNSWorker::get_src_ip() const {
    return m_src_ip;
}
...
void DNSWorker::worker() {
    ...
    while (!m_worker_finished) {
        std::unique_lock<std::mutex> lk(m_worker_mutex);
        m_worker_cond.wait_for(lk, 100ms, [this]{ return !m_tmp_websites.empty() || m_worker_finished; });
        ...
        for (auto& data : m_tmp_websites) {
            ...
            if (std::equal(www.begin(), www.end(), data.begin()) || std::equal(m.begin(), m.end(), data.begin())) {
