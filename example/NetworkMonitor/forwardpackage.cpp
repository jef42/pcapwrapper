...
template <typename IT, typename V> bool exists(IT begin, IT end, const V &v) {
...
ForwardPackage::ForwardPackage(PCAP::IpAddress local_ip,
    ...
    : m_local_ip{local_ip}, m_local_mac{local_mac}, m_router_ip{router_ip},
      m_router_mac{router_mac}, m_interface_name{interface_name},
      m_stop{false} {
    m_new_clients.reserve(10);
    m_existing_clients.reserve(10);
    m_future_clients = std::async(std::launch::async,
                                  &ForwardPackage::clients_receivers, this);
    m_future_working =
        std::async(std::launch::async, &ForwardPackage::working_function, this);
}

void ForwardPackage::new_client(PCAP::IpAddress target_ip,
...
void ForwardPackage::clients_receivers() {
    ...
    while (!m_stop) {
            ...
            return;
        {
            ...
            for_each(m_existing_clients.begin(), m_existing_clients.end(),
                     [this](auto &client) {
            ...
            for_each(m_new_clients.begin(), m_new_clients.end(),
                     [this](auto &client) {
...
void ForwardPackage::stop() {
    ...
    m_stop = true;
    {
        ...
        std::lock_guard<std::mutex> lock(m_mutex);
        for_each(m_existing_clients.begin(), m_existing_clients.end(),
                 [this](auto &client) { client.m_running = false; });
    ...
    while (!m_stop) {
        {
            ...
            m_existing_clients.erase(
                std::remove_if(m_existing_clients.begin(),
                               m_existing_clients.end(),
                               [](auto &client) { return !client.m_running; }),
                m_existing_clients.end());
        }
