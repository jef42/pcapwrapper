...
DNSEntry::DNSEntry(const std::string& website, std::chrono::time_point<std::chrono::system_clock> time)
    : m_website{website},
    m_time{time},
    m_counter{1}
{}
...
std::ostream& operator<<(std::ostream& stream, const DNSEntry& entry) {
    ...
    stream << entry.m_website << " " << entry.m_counter << " " << std::string(std::ctime(&time));
    return stream;
}
