...
void CookieWorker::new_package(PCAP::TCPPackage package) {
    std::string data =
        std::string((char *)package.get_data(), package.get_data_length());
...
PCAP::IpAddress CookieWorker::get_src_ip() const { return m_src_ip; }
...
std::string get_cookie(const std::string &header) {
    ...
    if (cookie_index != std::string::npos) {
        return std::string(header, cookie_index,
                           cookie_end == std::string::npos
                               ? header.size() - cookie_index
                               : cookie_end);
    }
    return "";
}
...
void CookieWorker::worker() {
    ...
    while (!m_worker_finished) {
        ...
        m_worker_cond.wait_for(lk, 100ms, [this] {
            return !m_payloads.empty() || m_worker_finished;
        });
        ...
        for (auto &payload : m_payloads) {
            ...
            if (index != std::string::npos) {
                ...
                if (std::find(std::begin(m_websites), std::end(m_websites),
                              website) == std::end(m_websites)) {
