...
MacAddress::MacAddress() { memset(m_mac.data(), 0xFF, ethernet_addr_len); }

bool operator==(const MacAddress &lhs, const MacAddress &rhs) noexcept {
    return lhs.m_mac[5] == rhs.m_mac[5] && lhs.m_mac[4] == rhs.m_mac[4] &&
           lhs.m_mac[3] == rhs.m_mac[3] && lhs.m_mac[2] == rhs.m_mac[2] &&
           lhs.m_mac[1] == rhs.m_mac[1] && lhs.m_mac[0] == rhs.m_mac[0];
}
