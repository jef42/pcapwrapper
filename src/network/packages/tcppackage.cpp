...
unsigned char TCPPackage::getTcpFlags() const { return m_tcp->m_th_flags; }

void TCPPackage::setTcpFlags(unsigned char flags) { m_tcp->m_th_flags = flags; }

unsigned int TCPPackage::getSeqNr() const { return ntohl(m_tcp->m_th_seq); }

unsigned int TCPPackage::getAckNr() const { return ntohl(m_tcp->m_th_ack); }

unsigned char TCPPackage::getDataOffset() const { return TH_OFF(m_tcp); }
...
void TCPPackage::setSeqNr(unsigned int nr) { m_tcp->m_th_seq = htonl(nr); }

void TCPPackage::setAckNr(unsigned int nr) { m_tcp->m_th_ack = htonl(nr); }
