...
void Processor::callback_impl(const unsigned char *package,
    ...
    case ETHER_TYPE::APR: {
        notifyListeners<ARPPackage>(m_arp_listeners.begin(),
                                    m_arp_listeners.end(), package, header.len);
        m_arp_listeners.erase(removeWeakPtr<ARPPackage>(m_arp_listeners.begin(),
                                                        m_arp_listeners.end()),
                              m_arp_listeners.end());
        break;
    }
    ...
    switch (ip->m_ip_p) {
    case IPPROTO_TCP: {
        notifyListeners<TCPPackage>(m_tcp_listeners.begin(),
                                    m_tcp_listeners.end(), package, header.len);
        m_tcp_listeners.erase(removeWeakPtr<TCPPackage>(m_tcp_listeners.begin(),
                                                        m_tcp_listeners.end()),
                              m_tcp_listeners.end());
        break;
    } break;
    case IPPROTO_UDP: {
        notifyListeners<UDPPackage>(m_udp_listeners.begin(),
                                    m_udp_listeners.end(), package, header.len);
        m_udp_listeners.erase(removeWeakPtr<UDPPackage>(m_udp_listeners.begin(),
                                                        m_udp_listeners.end()),
                              m_udp_listeners.end());
        break;
    }
    case IPPROTO_ICMP: {
        notifyListeners<ICMPPackage>(m_icmp_listeners.begin(),
                                     m_icmp_listeners.end(), package,
                                     header.len);
        m_icmp_listeners.erase(
            removeWeakPtr<ICMPPackage>(m_icmp_listeners.begin(),
...
void Processor::notifyListeners(IT begin, IT end,
...
void Processor::addSessionController(
...
void Processor::removeListener(
    const std::shared_ptr<PackageListener<TCPPackage>> &listener) {
    m_tcp_listeners.erase(
        removeWeakPtr(m_tcp_listeners.begin(), m_tcp_listeners.end(), listener),
        m_tcp_listeners.end());
}
void Processor::removeListener(
    const std::shared_ptr<PackageListener<ICMPPackage>> &listener) {
    m_icmp_listeners.erase(removeWeakPtr(m_icmp_listeners.begin(),
...
void Processor::removeListener(
    ...
    m_udp_listeners.erase(
        removeWeakPtr(m_udp_listeners.begin(), m_udp_listeners.end(), listener),
        m_udp_listeners.end());
}
void Processor::removeListener(
    const std::shared_ptr<PackageListener<ARPPackage>> &listener) {
    m_arp_listeners.erase(
        removeWeakPtr(m_arp_listeners.begin(), m_arp_listeners.end(), listener),
        m_arp_listeners.end());
}

void Processor::removeSessionController(
    const std::shared_ptr<SessionController> &controller) {
    m_tcp_listeners.erase(
        removeWeakPtr(m_tcp_listeners.begin(), m_tcp_listeners.end(),
                      static_cast<std::shared_ptr<PackageListener<TCPPackage>>>(
                          controller)),
        m_tcp_listeners.end());
    m_udp_listeners.erase(
        removeWeakPtr(m_udp_listeners.begin(), m_udp_listeners.end(),
